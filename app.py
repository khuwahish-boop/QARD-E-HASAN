...
def load_data():
    ...
    # Create repayments.csv if it doesn't exist
    rep_path = csv_path("repayments.csv")
    if not os.path.exists(rep_path):
        pd.DataFrame(columns=["repayment_id","loan_id","username","amount","date","note"]).to_csv(rep_path, index=False)
    repayments = pd.read_csv(rep_path)

    sch_path = csv_path("schedules.csv")
    if not os.path.exists(sch_path):
        pd.DataFrame(columns=["schedule_id","loan_id","username","installment_no","due_date","amount_due","status"]).to_csv(sch_path, index=False)
    schedules = pd.read_csv(sch_path)
    return users, loans, wallet, transactions, repayments, schedules

users, loans, wallet, transactions, repayments, schedules = load_data()
...
with st.sidebar:
    ...
    menu = st.radio(
        "Navigation",
        ["◈  Dashboard", "✦  Apply Loan", "◉  Wallet", "≡  Transactions", "☰  Repayments"],
...
wallet = pd.read_csv(csv_path("wallet.csv"))
transactions = pd.read_csv(csv_path("transactions.csv"))
repayments = pd.read_csv(csv_path("repayments.csv")) if os.path.exists(csv_path("repayments.csv")) else pd.DataFrame(columns=["repayment_id","loan_id","username","amount","date","note"])
schedules = pd.read_csv(csv_path("schedules.csv")) if os.path.exists(csv_path("schedules.csv")) else pd.DataFrame(columns=["schedule_id","loan_id","username","installment_no","due_date","amount_due","status"])
...
if page == "Dashboard":
    ...
    with chart_col1:
        ...
        if not user_txn.empty:
            ...
            fig_bar.update_layout(
                margin=dict(l=0, r=0, t=30, b=40), height=220,
                ...
                yaxis=dict(
                    gridcolor='#f0fdf9', showgrid=True,
                    tickfont=dict(size=11, color='#374151'),
                    tickformat=',.0f'
                )
            )
    ...
    with chart_col3:
        ...
        if not user_txn.empty:
            ...
            bal_df = bal_df.sort_values('date').reset_index(drop=True)
            ...
            bal_df['date_label'] = bal_df['date'].dt.strftime('%d %b %Y')
            fig_line = go.Figure()
            fig_line.add_trace(go.Scatter(
                x=bal_df['date_label'], y=bal_df['cumulative'],
                mode='lines+markers', line=dict(color='#0d9488', width=2.5),
                marker=dict(color='#0d9488', size=7),
                fill='tozeroy', fillcolor='rgba(13,148,136,0.08)',
                hovertemplate='%{x}<br>PKR %{y:,.0f}<extra></extra>'
            ))
            fig_line.update_layout(
                margin=dict(l=0, r=0, t=0, b=40), height=220,
                plot_bgcolor='#fff', paper_bgcolor='#fff',
                font=dict(family='Sora', size=12, color='#374151'),
                xaxis=dict(
        ...
        with col_action:
            if status == 'Pending':
                if st.button("Approve", key=f"approve_{row['loan_id']}"):
                    ...
                    existing_sch = schedules[schedules['loan_id'] == lid]
                    if existing_sch.empty:
                        from dateutil.relativedelta import relativedelta
                        installment_amt = round(amt / dur)
                        start_date = datetime.today()
                        new_rows = []
                        next_id = int(schedules['schedule_id'].max()) + 1 if not schedules.empty else 1
                        for i in range(dur):
                            due = (start_date + relativedelta(months=i+1)).strftime('%Y-%m-%d')
                            new_rows.append({
                                "schedule_id": next_id + i,
                                "loan_id": lid,
                                "username": user,
                                "installment_no": i + 1,
                                "due_date": due,
                                "amount_due": installment_amt,
                                "status": "Pending"
                            })
                        schedules_new = pd.concat([schedules, pd.DataFrame(new_rows)], ignore_index=True)
                        safe_save(schedules_new, "schedules.csv")

                    st.success(f"Loan #{lid:03d} approved. PKR {int(amt):,} credited. Repayment schedule generated.")
                    st.rerun()
...
# -------------------------
    ...
    st.markdown("""
    <div style="font-family:'Sora',sans-serif;font-size:24px;font-weight:600;color:#0f766e;margin-bottom:6px;">Loan Repayment Tracker</div>
    <div style="font-size:13px;color:#6b7280;margin-bottom:24px;">Log your installments and track remaining balances.</div>
    """, unsafe_allow_html=True)
    ...
    if approved_loans.empty:
        st.markdown("""
        <div style="background:#fff;border:0.5px solid #d1fae5;border-radius:14px;padding:32px;text-align:center;">
            <div style="font-size:14px;font-weight:500;color:#6b7280;">No approved loans found.</div>
            <div style="font-size:12px;color:#9ca3af;margin-top:6px;">Only approved loans can have repayments tracked.</div>
        </div>
        """, unsafe_allow_html=True)
    else:
        # ── Summary metrics ──
        user_repayments = repayments[repayments['username'] == st.session_state.user]
        total_borrowed_approved = int(approved_loans['amount'].sum())
        total_repaid = int(user_repayments['amount'].sum()) if not user_repayments.empty else 0
        remaining = max(total_borrowed_approved - total_repaid, 0)
        repaid_pct = min(int((total_repaid / total_borrowed_approved) * 100), 100) if total_borrowed_approved > 0 else 0

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Total Borrowed", f"PKR {total_borrowed_approved:,}")
        c2.metric("Total Repaid",   f"PKR {total_repaid:,}")
        c3.metric("Remaining",      f"PKR {remaining:,}")
        c4.metric("Repaid",         f"{repaid_pct}%")
        ...
        for _, loan in approved_loans.iterrows():
            ...
            loan_reps = user_repayments[user_repayments['loan_id'] == loan_id]
            repaid    = int(loan_reps['amount'].sum()) if not loan_reps.empty else 0
            rem       = max(loan_amt - repaid, 0)
            pct       = min(int((repaid / loan_amt) * 100), 100) if loan_amt > 0 else 0
            monthly_target = round(loan_amt / duration)
            installments_done = len(loan_reps)

            bar_color = "#0d9488" if pct < 100 else "#059669"

            st.markdown(f"""
            <div style="background:#fff;border:0.5px solid #d1fae5;border-radius:14px;padding:22px 26px;margin-bottom:16px;">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:14px;">
                    <div>
                        <div style="font-size:14px;font-weight:600;color:#111827;">Loan #{loan_id:03d}</div>
                        <div style="font-size:12px;color:#6b7280;margin-top:2px;">{duration} months · PKR {loan_amt:,} total · PKR {monthly_target:,}/month suggested</div>
                    </div>
                    <div style="font-size:11px;font-weight:600;padding:4px 12px;border-radius:20px;background:#d1fae5;color:#065f46;">
                        {"Settled" if pct == 100 else "Active"}
                    </div>
                </div>
                <div style="display:flex;justify-content:space-between;font-size:12px;color:#6b7280;margin-bottom:6px;">
                    <span>Repaid: <b style="color:#0d9488;">PKR {repaid:,}</b></span>
                    <span>Remaining: <b style="color:#374151;">PKR {rem:,}</b></span>
                    <span>{pct}% complete</span>
                </div>
                <div style="background:#f0fdf9;border-radius:999px;height:10px;overflow:hidden;">
                    <div style="background:{bar_color};width:{pct}%;height:100%;border-radius:999px;transition:width 0.4s;"></div>
                </div>
                <div style="font-size:11px;color:#9ca3af;margin-top:6px;">{installments_done} installment(s) recorded</div>
            </div>
            """, unsafe_allow_html=True)

            # ── Repayment Schedule for this loan ──
            loan_schedule = schedules[schedules['loan_id'] == loan_id].copy() if not schedules.empty else pd.DataFrame()
            if not loan_schedule.empty:
                # Mark installments as Paid based on logged repayments
                loan_schedule = loan_schedule.sort_values('installment_no').reset_index(drop=True)
                cumulative_repaid = 0
                for idx2, srow in loan_schedule.iterrows():
                    cumulative_repaid += srow['amount_due']
                    if repaid >= cumulative_repaid:
                        loan_schedule.at[idx2, 'status'] = 'Paid'
                    else:
                        today_str = datetime.today().strftime('%Y-%m-%d')
                        if srow['due_date'] < today_str:
                            loan_schedule.at[idx2, 'status'] = 'Overdue'
                        else:
                            loan_schedule.at[idx2, 'status'] = 'Pending'

                with st.expander(f"Repayment Schedule — Loan #{loan_id:03d}"):
                    # Header
                    st.markdown("""
                    <div style="display:grid;grid-template-columns:60px 1fr 1fr 1fr;gap:8px;padding:8px 12px;background:#f0fdf9;border-radius:8px;margin-bottom:8px;">
                        <div style="font-size:11px;font-weight:600;color:#6b7280;">No.</div>
                        <div style="font-size:11px;font-weight:600;color:#6b7280;">Due Date</div>
                        <div style="font-size:11px;font-weight:600;color:#6b7280;">Amount</div>
                        <div style="font-size:11px;font-weight:600;color:#6b7280;">Status</div>
                    </div>
                    """, unsafe_allow_html=True)

                    for _, srow in loan_schedule.iterrows():
                        s = srow['status']
                        if s == 'Paid':
                            s_bg, s_color, s_dot = "#d1fae5", "#065f46", "#10b981"
                        elif s == 'Overdue':
                            s_bg, s_color, s_dot = "#fee2e2", "#991b1b", "#ef4444"
                        else:
                            s_bg, s_color, s_dot = "#fef3c7", "#92400e", "#f59e0b"

                        st.markdown(f"""
                        <div style="display:grid;grid-template-columns:60px 1fr 1fr 1fr;gap:8px;padding:8px 12px;border-bottom:0.5px solid #f3f4f6;align-items:center;">
                            <div style="font-size:12px;font-weight:600;color:#374151;">#{int(srow['installment_no'])}</div>
                            <div style="font-size:12px;color:#374151;">{srow['due_date']}</div>
                            <div style="font-size:12px;font-family:'JetBrains Mono',monospace;color:#111827;">PKR {int(srow['amount_due']):,}</div>
                            <div><span style="font-size:11px;font-weight:600;padding:3px 10px;border-radius:20px;background:{s_bg};color:{s_color};">{s}</span></div>
                        </div>
                        """, unsafe_allow_html=True)

                    # Summary row
                    paid_count = len(loan_schedule[loan_schedule['status'] == 'Paid'])
                    overdue_count = len(loan_schedule[loan_schedule['status'] == 'Overdue'])
                    pending_count = len(loan_schedule[loan_schedule['status'] == 'Pending'])
                    st.markdown(f"""
                    <div style="display:flex;gap:16px;margin-top:12px;padding:10px 12px;background:#f9fafb;border-radius:8px;">
                        <div style="font-size:12px;color:#059669;font-weight:500;">Paid: {paid_count}</div>
                        <div style="font-size:12px;color:#d97706;font-weight:500;">Pending: {pending_count}</div>
                        <div style="font-size:12px;color:#dc2626;font-weight:500;">Overdue: {overdue_count}</div>
                    </div>
                    """, unsafe_allow_html=True)

            # Installment history for this loan
            if not loan_reps.empty:
                with st.expander(f"View installment history — Loan #{loan_id:03d}"):
                    for _, rep in loan_reps.sort_values('date', ascending=False).iterrows():
                        note_text = f" · {rep['note']}" if pd.notna(rep.get('note')) and str(rep.get('note','')) not in ['', 'nan'] else ''
                        st.markdown(f"""
                        <div style="display:flex;align-items:center;gap:12px;padding:8px 0;border-bottom:0.5px solid #f3f4f6;">
                            <div style="width:30px;height:30px;border-radius:50%;background:#d1fae5;display:flex;align-items:center;justify-content:center;font-size:13px;">↑</div>
                            <div style="flex:1;">
                                <div style="font-size:13px;font-weight:500;color:#111827;">PKR {int(rep['amount']):,}</div>
                                <div style="font-size:11px;color:#6b7280;">{rep['date']}{note_text}</div>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
        ...
        # ── Log a new repayment ──
        st.markdown("""
        <div style="background:#fff;border:0.5px solid #d1fae5;border-radius:14px;padding:22px 26px;">
            <div style="font-size:14px;font-weight:600;color:#374151;margin-bottom:16px;">Log a Repayment Installment</div>
        """, unsafe_allow_html=True)

        loan_options = {f"Loan #{int(r['loan_id']):03d} — PKR {int(r['amount']):,} ({int(r['duration'])} months)": int(r['loan_id'])
                        for _, r in approved_loans.iterrows()}

        st.markdown("<div style='font-size:13px;font-weight:500;color:#374151;margin-bottom:4px;'>Select Loan</div>", unsafe_allow_html=True)
        selected_label = st.selectbox("Select Loan", list(loan_options.keys()), label_visibility="collapsed", key="rep_loan_select")
        selected_loan_id = loan_options[selected_label]

        col_a, col_b = st.columns(2)
        with col_a:
            st.markdown("<div style='font-size:13px;font-weight:500;color:#374151;margin-bottom:4px;'>Amount Paid (PKR)</div>", unsafe_allow_html=True)
            rep_amount = st.number_input("Amount", min_value=1, step=500, value=1000, label_visibility="collapsed", key="rep_amount")
        with col_b:
            st.markdown("<div style='font-size:13px;font-weight:500;color:#374151;margin-bottom:4px;'>Date</div>", unsafe_allow_html=True)
            rep_date = st.date_input("Date", value=datetime.today(), label_visibility="collapsed", key="rep_date")

        st.markdown("<div style='font-size:13px;font-weight:500;color:#374151;margin-bottom:4px;margin-top:10px;'>Note (optional)</div>", unsafe_allow_html=True)
        rep_note = st.text_input("Note", placeholder="e.g. Monthly installment, partial payment", label_visibility="collapsed", key="rep_note")

        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

        # Validate against remaining
        sel_loan_row = approved_loans[approved_loans['loan_id'] == selected_loan_id].iloc[0]
        sel_loan_amt = int(sel_loan_row['amount'])
        sel_repaid   = int(user_repayments[user_repayments['loan_id'] == selected_loan_id]['amount'].sum()) if not user_repayments.empty else 0
        sel_rem      = max(sel_loan_amt - sel_repaid, 0)

        st.markdown(f"""
        <div style="background:#f0fdf9;border:0.5px solid #a7f3d0;border-radius:10px;padding:10px 14px;margin-bottom:14px;">
            <div style="font-size:12px;color:#065f46;font-weight:500;">
                Remaining on selected loan: PKR {sel_rem:,}
            </div>
        </div>
        """, unsafe_allow_html=True)

        if st.button("Record Installment", key="btn_record_rep"):
            if rep_amount <= 0:
                st.error("Enter a valid amount.")
            elif rep_amount > sel_rem:
                st.error(f"Amount exceeds remaining balance of PKR {sel_rem:,}.")
            else:
                new_rep_id = int(repayments['repayment_id'].max()) + 1 if not repayments.empty else 1
                new_rep = {
                    "repayment_id": new_rep_id,
                ...
                repayments.loc[len(repayments)] = new_rep
                safe_save(repayments, "repayments.csv")

                # Deduct from wallet
                wallet.loc[wallet['username'] == st.session_state.user, 'balance'] -= rep_amount
                safe_save(wallet, "wallet.csv")

                # Mark loan as Settled if fully repaid
                new_total_repaid = sel_repaid + rep_amount
                if new_total_repaid >= sel_loan_amt:
                    loans.loc[loans['loan_id'] == selected_loan_id, 'status'] = "Settled"
                    safe_save(loans, "loans.csv")
                    st.success(f"Installment of PKR {rep_amount:,} recorded. Loan #{selected_loan_id:03d} is now fully settled.")
                else:
                    st.success(f"Installment of PKR {rep_amount:,} recorded. Remaining: PKR {sel_rem - rep_amount:,}")
                st.rerun()

        st.markdown("</div>", unsafe_allow_html=True)

        # ── Repayment trend chart ──
        if not user_repayments.empty:
            st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
            st.markdown("""
            <div style="background:#fff;border:0.5px solid #d1fae5;border-radius:14px;padding:22px 26px;">
                <div style="font-size:14px;font-weight:600;color:#374151;margin-bottom:4px;">Repayment History</div>
                <div style="font-size:12px;color:#6b7280;margin-bottom:12px;">Cumulative amount repaid over time</div>
            """, unsafe_allow_html=True)

            chart_df = user_repayments.copy()
            chart_df['date'] = pd.to_datetime(chart_df['date'], errors='coerce')
            chart_df = chart_df.sort_values('date')
            chart_df['cumulative'] = chart_df['amount'].cumsum()
            chart_df['date_label'] = chart_df['date'].dt.strftime('%d %b %Y')

            fig_rep = go.Figure()
            fig_rep.add_trace(go.Scatter(
                x=chart_df['date_label'], y=chart_df['cumulative'],
                mode='lines+markers',
                line=dict(color='#0d9488', width=2.5),
                marker=dict(color='#0d9488', size=7),
                fill='tozeroy', fillcolor='rgba(13,148,136,0.08)',
                hovertemplate='%{x}<br>PKR %{y:,.0f} repaid<extra></extra>'
            ))
            # Add total borrowed line
            fig_rep.add_hline(
                y=total_borrowed_approved,
                line_dash="dash", line_color="#f87171",
                annotation_text="Total Borrowed",
                annotation_position="top right",
                annotation_font_color="#f87171"
            )
            fig_rep.update_layout(
                margin=dict(l=0, r=0, t=20, b=40), height=240,
                plot_bgcolor='#fff', paper_bgcolor='#fff',
                font=dict(family='Sora', size=12, color='#374151'),
                xaxis=dict(showgrid=False, tickangle=-30, tickfont=dict(size=11), automargin=True),
                yaxis=dict(gridcolor='#f0fdf9', tickformat=',.0f', tickfont=dict(size=11), title='PKR')
            )
